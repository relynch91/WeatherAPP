...
export async function getWeather(formatted) {
    let geoKey = await axios.get("/location/getHeroku");
    // let googleKey = process.env.GOOGLE_GEO_CODING;
    let latLong = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${formatted}&key=${geoKey.data}`);
